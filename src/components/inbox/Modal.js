...
export default function Modal({ open, control }) {
  ...
  const { data,} = useGetConversationsQuery(myEmail) || {}
  ...
  }, [participant, dispatch, myEmail, to, conversation])
  ...
  const handleSubmit =async (e) => {
    ...
    const audit= conversations.find(item => item.participants.includes(participant[0].email));
    if(!audit){
      addConversation({
        sender: myEmail,
        data: {
